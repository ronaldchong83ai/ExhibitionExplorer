...
export async function POST(request: NextRequest) {
  try {
    ...
    const stored = otpStore.get(`register_otp:${email.toLowerCase()}`);
    if (!stored) {
    ...
    if (Date.now() > stored.expiresAt) {
      otpStore.delete(`register_otp:${email.toLowerCase()}`);
      return Response.json({ success: false, error: 'OTP has expired. Please request a new one.' }, { status: 400 });
    }

    // Set verified flag for final account creation check
    otpStore.set(`register_verified:${email.toLowerCase()}`, true);
